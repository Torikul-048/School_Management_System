...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Event</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Update event information</p>
        </div>
        ...
        <form action="{{ route('events.update', $event) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">{{ old('description', $event->description) }}</textarea>
                </div>
                ...
                @if($event->image)
                    <div class="md:col-span-2">
                        ...
                        <img src="{{ Storage::url($event->image) }}" alt="{{ $event->title }}" class="w-48 h-32 object-cover rounded-lg">
                    </div>
                @endif

                <!-- New Image -->
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ $event->image ? 'Replace Image' : 'Event Image' }}</label>
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="send_notification" value="1" {{ old('send_notification') ? 'checked' : '' }}
                               class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Send notification about changes</span>
                    </label>
                </div>
            </div>
